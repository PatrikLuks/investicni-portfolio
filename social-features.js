...
class SocialFeatures {
  ...
  createSocialUI() {
    ...
    socialBtn.style.cssText = 'font-size: 1.5rem; padding: 8px 16px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 8px; cursor: pointer;';
  ...
  renderSharedPortfolios() {
    const myShares = Array.from(this.sharedPortfolios.values())
      .filter(share => share.userId === this.currentUser.id);
  ...
  updateSocialUI() {
    ...
    if (panel && panel.style.display !== 'none') {
      ...
      if (activeTab) {
        ...
        if (tabId === 'tabFeed') this.showFeedTab();
        else if (tabId === 'tabShare') this.showShareTab();
        else if (tabId === 'tabExplore') this.showExploreTab();
      }
    }
  }
